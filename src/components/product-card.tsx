...
export default function ProductCard({ product }: ProductCardProps) {
   return (
      <div className='relative col-span-1 overflow-hidden'>
         ...
         <img src={product.image} alt={`${product.title} image`} className='aspect-square object-cover rounded-lg' />
         <div className='flex items-center justify-between mt-2'>
            <h4 className='sm:text-lg opacity-80'>{product.title}</h4>
            <p className='sm:text-lg text-muted-foreground font-sans font-medium'>${product.price}</p>
         </div>
         <p className='text-muted-foreground text-sm'>{product.rating.rate} star rating</p>
      </div>
   )
}
